...
<<<<<<< HEAD
# Relative file path (friendly for users regardless of machine) relative to working directory
...
#type read.csv( inset quotes, then period, then slash, then TAB, and files will come up
...
> ANSWER: Located in the metadata folder. Separated from the raw data. 
...
str(USGS.flow.data)
# shows data frame and all options within
dim(USGS.flow.data)
# shows how many rows and columns 
length(USGS.flow.data)
#shows how many columsn

head(USGS.flow.data)
#shows first few rows of data, can specify
...
> Answer: time series - sometimes with older datasets a lot of missing observations, dont remove bc typically in middle of data. For other types may be easier to omit 

```{r removeNA}
USGS.flow.data.complete <- na.omit(USGS.flow.data)
#create new object to store data without NA's
dim(USGS.flow.data)
