...
O3plot2 <- ggplot(EPAair) +
  ...
  ### have to put color outside of aes 
  geom_point(aes(x = Date, y = Ozone), color = "blue")
print(O3plot2)
...
PMplot <- 
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year), color = Site.Name)) + # color assigned to a variable, now inside aes, X is now month
  geom_point()
print(PMplot)
...
PMplot.line <- 
  ...
  geom_line()
print(PMplot.line)
#changed x back to date bc line is a continuous variable, so have to use Data and not month
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() #if dont use this, will go from 0 to max, but depth is on a reverse scale, so want 0 on top and deepest on bottom part of y axis
print(tempvsdepth)
...
NvsP <-
  ...
  geom_abline(aes(slope = 16, intercept = 0)) # if you knew slope and intercept 
print(NvsP)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? - fill just changes fill of box plot, but contour remains black. color is way to go when working with boxplots 
print(Nutrientplot3)
# much better than bar with error 
...
print(Nutrientplot4)
#stacking points based on center position. position dodge making observations at same concentration go to side rather than overlay each other 
...
#^example of not a good plot 

# Using a gathered dataset, better example 
Nutrientplot7 <-   
  ggplot(PeterPaul.chem.nutrients.gathered) +
  geom_freqpoly(aes(x = concentration, color = nutrient))
print(Nutrientplot7)
#because we gathered nutrients in these two columns, much easier 
...
# Ridgeline plot, will show density of distribution over time 
Nutrientplot6 <-
