...
unique(NTL.phys.data$lakename)## different way of seeing lakenames
...
summary(NTL.phys.data$daynum)## cant list unique values bc numeric 
...
NLT.phys.data.1999 <- filter(NTL.phys.data, year4 == 1999)
...
NLT.phys.data.90_99 <- filter(NTL.phys.data, year4 >= 1990 & year4 <=1999 & lakename == "Tuesday Lake")

## use & instead of commas

NTL.phys.data.90.99.2 <- 
  NTL.phys.data %>%
...
## drop na when look at every row, find null values, then eliminate those rows
## a more subtle way to do it is is.na, will ask value in temp column and see if null, if so will filter out 
...
NTL.phys.data.longer <- pivot_longer(NTL.phys.data, irradianceWater:irradianceDeck, names_to = "irradiance_type", values_to = "irradiance")
...
NTL.phys.data.wider <- 
  NTL.phys.data.longer %>%
  pivot_wider(names_from = irradiance_type, values_from = irradiance)
