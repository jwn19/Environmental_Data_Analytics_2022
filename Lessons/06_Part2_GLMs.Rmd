...
# Test for normality. It was one of the assumptions. 
...
#Most nulls are rejected. Do not follow a normal distribution. does not mean you cannot run a one way anova test
#for shapiro test, have to call an object, then filter
...
#can see data does not follow normal distribution
...
#shows number of factors (degrees of freedom +1), p value is less than 0.05, reject null, which is the mean is the same for all plot id's, so we are saying that they are not all the same 
#it is not telling us which pair of means are not the same
#have to run other tests 
...
#runs through all 12 pairs against each other 
#shows difference between means 
TukeyHSD(Litter.Totals.anova)

#follow up for tukeyhsd test 
# Extract groupings for pairwise relationships
Litter.Totals.groups <- HSD.test(Litter.Totals.anova, "plotID", group = TRUE)

#group = true saying group levels with same mean 
Litter.Totals.groups

#groups with same letter are not significantly different, indicating same mean. 
...
> first thing to do is reorder sites according to previous groups list. 
...
#reject null hypothesis, mean of our groups is different

#looking at 2 categorical variables on a continuous response variable 
...
#using times to see interaction 
summary(Litter.anova.2way3)

#p value less than 0.05, interaction is significant 
...
#now want to find treatment groups like we did with one way anova
#will have 24 levels bc of interaction with functional group and class

Litter.interaction <- with(Litter, interaction(functionalGroup, nlcdClass))

#cbind used as example to show  what we are doing. dont need to create this

Litter.test <- cbind(Litter,Litter.interaction)

Litter.anova.2way5 <- aov(data = Litter, dryMass ~ Litter.interaction)
#comparing means for each one of 24 interaction levels. technically a one way inova, but based on interaction of two categorical variables
