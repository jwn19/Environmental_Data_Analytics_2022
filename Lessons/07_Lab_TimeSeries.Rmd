...
temp_ts <- ts(BRA_complete$Temperature_C,
                   start=c(f_year_rain,f_month_rain),
                   frequency=12)
...
rain_decomp_components <- as.data.frame(rain_decomp$time.series[,1:3])

rain_decomp_components <- mutate(rain_decomp_components,
        Observed = BRA_complete$Rainfall,     
        Date = Rainfall_BR_processed$Date)

temp_decomp_components <- as.data.frame(temp_decomp$time.series[,1:3])
