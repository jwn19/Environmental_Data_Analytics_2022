...
# Evaluate assumption of normal distribution. reject the null (that it is normal, so we are actually saying it is nonnormal )
...
year1819 <- as.factor(EPAair$Year)

Density <- ggplot(EPAair, aes(x = Ozone)) +
 geom_density(aes(fill = year1819, alpha = 0.5))
print(Density)
...
epa.regression <- lm(data = EPAair, PM2.5 ~ Ozone)
summary(epa.regression)


EPA2 <- EPAair %>%
  na.omit()

epa.regression2 <- lm(data = EPA2, PM2.5 ~ Ozone)
summary(epa.regression2)
...
the_cor <- cor(EPAair %>% select(meanLat:Ozone)%>% drop_na())
