...
## changed to date object, will be useful in case want to select date range or sort
class(USGS.flow.data$datetime)
...
### bar plot useful for looking at counts of categorical data!! 
...
#added additonal argument specific to binwidth. Default is 30 
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

#another aesthetic change. generally between 5-20 bins is the best. if you have more might show patterns that dont exist (showing noise rather than signal)
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

#changing x axis, way of zooming into histogram where most data is 
...
```{r, fig.height = 3, fig.width = 4}
#look at cheatsheets! can find under help 
ggplot(USGS.flow.data) +
...
#coloring by mean approval (categorical variable); use to show two categories of data on same plot 
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#categorical and numeric data. analogy to bar plot 
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

#taking different slices of data and looking at distribution. analogous to histogram, but looking at distribution rather than counts 
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

#can look at range of values and distribution within that range. width is how many data points are distributed within 
ggplot(USGS.flow.data) +
...
ggplot(na.omit(USGS.flow.data) + geom_histogram(aes(x = discharge.mean)))
...
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.max, y = discharge.max, group = cut_width(gage.height.mean, 2)))

install.packages("DataExplorer")

library(DataExplorer)

DataExplorer::plot_density(USGS.flow.data$discharge.mean)
