...
EPAair_PM25_NC2018_subset <- 
  EPAair_PM25_NC2018 %>%
  select(Date, DAILY_AQI_VALUE,Site.Name,AQS_PARAMETER_DESC,COUNTY,SITE_LATITUDE,SITE_LONGITUDE)

EPAair_PM25_NC2019_subset <- 
  EPAair_PM25_NC2019 %>%
  select(Date, DAILY_AQI_VALUE,Site.Name,AQS_PARAMETER_DESC,COUNTY,SITE_LATITUDE,SITE_LONGITUDE)
...
write.csv(EPAair_O3_NC2018_subset, row.names = FALSE, file = "./Data/Processed/EPAair_03_NC2018_processed.csv")
write.csv(EPAair_O3_NC2019_subset, row.names = FALSE, file = "./Data/Processed/EPAair_03_NC2019_processed.csv")
...
dim(EPAair_Combined_commonsites)
...
write.csv(EPAair_Combined_spread, row.names = FALSE, file = "./Data/Processed/EPAair_O3_PM25_NC2122_Processed.csv")
...
EPAair_Summary_clean <- 
  EPAair_Combined_spread%>%
  group_by(Site.Name, Month, Year) %>%
  summarise(meanAQI_O3 = mean(Ozone), 
            meanAQI_PM2.5 = mean(PM2.5))%>%
  filter(is.na(meanAQI_O3&meanAQI_PM2.5))
...
dim(EPAair_Summary_clean)

```

14. Why did we use the function `drop_na` rather than `na.omit`?

> Answer: I used is.na because the drop.na function would not let me use an & operator. I am unsure why we would use drop_na rather than na.omit, other than because it is a tidyverse function. 
