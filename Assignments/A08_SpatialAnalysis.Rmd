...
> ANSWER: EPSG code is 4269. Geographic coordinate system. 
> 
...
gage_height <- read.csv("./Data/Raw/NWIS_SiteFlowData_NE_RAW.csv", stringsAsFactors = TRUE) 
...
ggplot() +
  geom_sf(data = counties_sf,color='blue') + 
  geom_sf(data = gage_full_sf, aes(color = gage_ht))+
...
gage_site_sf_utm <- st_transform(gage.sf, crs = 32614)
...
ggplot() +
  ...
  geom_sf(data = gage_site_sf_utm,color='red')
...
gage_intersect_1 <- gage_site_sf_utm[lancaster_sf,]


#24 Plot

mapview(gage_intersect_1)
#unclear why this will not show up in pdf, but it is showing up fine in R
